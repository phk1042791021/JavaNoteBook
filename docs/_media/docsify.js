(function () {
   ...
   * accordingly. */
    ...
    function callHook(vm, hookName, data, next) {
        if ( next === void 0 ) next = noop;

        var queue = vm._hooks[hookName];
    ...
    function collapse(el) {
        ...
        on(el, 'click', function (ref) {
            ...
            if (
                ...
                if (!target.classList.contains('collapse_transition')){
                    toggleClass(target, 'collapse_transition')
                }
                // toggleClass(target.nextSibling, 'collapse_open');
    ...
    function tree(toc, tpl) {
        var level = 0
        if ( tpl === void 0 ) {
            if (level===2) tpl = '<ul class="app-sub-sidebar" style="display: none">{inner}</ul>';
  ...
  ********************************************** */
        ...
        proto._renderMain = function(text, opt, next) {
            ...
            callHook(this, 'beforeEach', text, function (result) {
                ...
                var callback = function () {
                    ...
                    callHook(this$1, 'afterEach', html, function (hookData) { return renderMain.call(this$1, hookData); }
                    );
                };
    ...
    var HashHistory = /*@__PURE__*/(function (History) {
        ...
        HashHistory.prototype.onchange = function onchange (cb) {
            ...
            on('click', function (e) {
                // var ele = getNode(e.target)
                var el = e.target.tagName === 'A' ? e.target : e.target.parentNode;
                // if (el && el.tagName === 'A' && !el.classList.contains("collapse_transition")){
                //     console.log(el.getAttribute("href"));
                //
                //     console.log(ele)
                //     ele.setAttribute("class","collapse_transition")
                //     // toggleClass(ele, 'collapse_transition')
                //
                // }
